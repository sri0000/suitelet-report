  ...
  function onRequest(context) {
    try {
      ...
      if (selectedPeriod) {
        ...
        //2year before April to Selected Month Date format
        var sMI = monthMap[monthName];// Assuming fiscal year starts in March (zero-based index: 2) (SMI selected Month index)
        var fSM = 3; // March (FSM -> fiscal Start Month)
        var fEM = sMI;   // Also March (fixed period) (FEM -> Fiscal End Month)

        // Determine fiscal year logic based on selected month
        var fYS; //(FYS -> fiscal year start)
        var fYE;  //( FYE -> fiscal year End)

        if (sMI < fSM) {
          fYS = year - 3; // Fiscal year starts three years before for months before March
          fYE = year;  // Fiscal year ends 2 year before
        } else {
          fYS = year - 2; // Fiscal year starts one year before for months March and after
          fYE = year;       // Fiscal year ends in the current year
        }

        // Calculate the fiscal start and end dates
        var march_FiscalStartDate = new Date(fYS, fSM, 1); // March 1st of fiscal start year
        var march_FiscalEndDate = new Date(fYE, fEM + 1, 0);  // March 31st of fiscal end year

        // Format the dates
        var formatted_April_FiscalStartDate = format.format({
          value: march_FiscalStartDate,
          type: format.Type.DATE
        });
        var formatted_cur_month_FiscalEndDate = format.format({
          value: march_FiscalEndDate,
          type: format.Type.DATE
        });

        // Log the results
        log.debug('April to current month Year Start Date:', formatted_April_FiscalStartDate);
        log.debug('April to current month Year End Date:', formatted_cur_month_FiscalEndDate);
        ...
        var invoiceSearchObj = search.create({
          ...
          columns: [
            ...
            search.createColumn({
              name: "formulacurrency", summary: "SUM", formula: "CASE      WHEN {taxtotal} IS NULL THEN {totalamount}     ELSE {totalamount} - {taxtotal} END",
        ...
        // 2 years before April to current month  new dealers amount
        // sales for the fical period start and end month for new dealers
        var cumFiscalNewDealersAmount = {};
        var invoiceSearchObj = search.create({
          type: "invoice",
          ...
          filters: [
            ...
            ["salesrep", "anyof", salesRepIds],
            "AND",
            ["trandate", "within", formatted_April_FiscalStartDate, formatted_cur_month_FiscalEndDate],
            "AND",
            ["customer.custentity_customer_date_creations_", "is", "T"]
          ],
          columns: [
        ...
        log.debug("2year April to Selected month  Sales Rep Amounts", cumFiscalNewDealersAmount); // Logs amounts for each sales rep ID
        ...
        var new_Dealers_sales_2years_total = 0;
        var new_Dealers_sales_2years = 0;
        var sales_Achive_v_target = 0;
        //3rd table variable
        ...
        for (var i = 0; i < internalIds.length; i++) {
          ...
          newDealersSalesTarget = fiscalCumTotal * 0.06;
          ...
          cum_amt_total = cumMonTotalAmount !== 0 ? (cum_AmountForRep / cumMonTotalAmount) * 100 : 0;
          total_cum_amt_percentage = cumMonTotalAmount !== 0 ? (cumMonTotalAmount / cumMonTotalAmount) * 100 : 0;

          new_Dealers_sales_2years = parseFloat(cumFiscalNewDealersAmount[salesRepId]) || 0;
          new_Dealers_sales_2years_total += new_Dealers_sales_2years;

          sales_Achive_v_target = newDealersSalesTarget !== 0 ? new_Dealers_sales_2years_total / newDealersSalesTarget : 0;
          ...
          salesVstarget = forTheMonth ? (cum_AmountForRep / forTheMonth) * 100 : 0;
          totalSalesVstarget = monthTotal !== 0 ? (cumMonTotalAmount / monthTotal) * 100 : 0;


          //2table e24 calculation
          percentSalesVsTarget = avgDayTotal * completedWorkDay * 0.06;
          //2nd table sales vs target
          // Avoid division by zero
          perDealerAchievedVsTarget = percentSalesVsTarget !== 0 ? (cumNewDealerTotal / percentSalesVsTarget) * 100 : 0;
          ...
          perSalesAchivedvsTarget = target_15_0f_sales !== 0 ? (cum_newProductMonth / target_15_0f_sales) * 100 : 0;
          totalPerSales = target_15_total !== 0 ? (cumNewProductTotal / target_15_total) * 100 : 0; // Add to total
          ...
          var tot_deal_Sm_Wise = totalNoOfDealers === 0 ? 0 : (group3Count / totalNoOfDealers) * 100; // total dealer sales man wise
          sm_Wise_Total = totalNumberOfDealersTotal === 0 ? 0 : (totalGroup3Count / totalNumberOfDealersTotal) * 100;
          td3 = '<td  style="width: 15px;height:10px;  padding: 6px; border-right: 1px solid black; border-bottom: 1px solid black; align: center; border-left: 1px solid black;">' + serialNumber + '</td>' +
            ...
            '<td  style="width: 15px;height:10px;  padding: 6px; align: center;  border-bottom: 1px solid black; border-right: 1px solid black;">' + formatNumber(tot_deal_Sm_Wise) + '%' + '</td>';
          ...
          td4 = '<td style="width: 15px;height:10px; padding: 6px; align: center; border-right: 1px solid black; border-bottom: 1px solid black; border-left: 1px solid black;">' + serialNumber + '</td>' +
            ...
            '<td style="width: 15px;height:10px; padding: 6px; align: center; border-right: 1px solid black; border-bottom: 1px solid black; background-color: rgb(137, 211, 127);">' + formatNumber(surplus) + '</td>' +
            '<td style="width: 15px;height:10px; padding: 6px; align: center; border-right: 1px solid black; border-bottom: 1px solid black; ">' + formatNumber(for_The_Day) + '</td>' +
        ...
        targetFor6Percent = (total_sales_target_for / 100) * 0.06;
...
<table  style="width: 100%; border-collapse: collapse; font-family: Arial, sans-serif; font-size: 12px; top:1%;">\
      ...
      <tr >\
        ...
        <td style="width: 50%;">\
          <table border="1" style="width: 100%; border-collapse: collapse; font-family: Arial, sans-serif; font-size: 12px;">\
            ...
            <tr style="width: 10%;height:2%;">\
              ...
              <td border-right="1"  style="width: 15px;height:10px; border-top: 1px solid black; padding: 6px; align: center; background-color: rgb(137, 211, 127);">'+ formatNumber(new_Dealers_sales_2years_total) + '</td>\
              <td style="width: 15px; height:10px; border-top: 1px solid black; padding: 6px;align: center;">'+ formatNumber(sales_Achive_v_target) + '%' + '</td>\
            </tr>\
          </table>\
        </td>\
      </tr>\
          ...
          <table  style="width: 100%; margin-top:20px; font-family: Arial, sans-serif; font-size: 11px; ">\
           ...
           '+ tr3 + '\
            <tr style="width: 10%;height:2%;">\
              ...
              <td  style="width: 15px;height:10px;  padding: 6px; align: center; border-bottom: 1px solid black; font-weight: bold; border-right: 1px solid black;">'+ formatNumber(sm_Wise_Total) + '%' + '</td>\
            </tr>\
          </table>\
       ...
       <tr style="width: 10%;height:2%;">\
          ...
          <td  style="width: 15px;height:10px;  padding: 6px; align: center; border-right: 1px solid black; border-bottom: 1px solid black; font-weight: bold;">'+ formatNumber(surplus_Total) + '</td>\
          <td  style="width: 15px;height:10px;  padding: 6px; align: center; border-right: 1px solid black; border-bottom: 1px solid black; font-weight: bold;">'+ formatNumber(for_The_Day_Total) + '</td>\
          <td  style="width: 15px;height:10px;  padding: 6px; align: center; border-right: 1px solid black; border-bottom: 1px solid black; font-weight: bold;">\</td>\
          <td  style="width: 15px;height:10px;  padding: 6px; align: center; border-right: 1px solid black; border-bottom: 1px solid black; font-weight: bold;">\</td>\
          <td  style="width: 15px;height:10px;  padding: 6px; align: center; border-right: 1px solid black; border-bottom: 1px solid black; font-weight: bold;">\</td>\
          <td  style="width: 15px;height:10px;  padding: 6px; align: center; border-right: 1px solid black; border-bottom: 1px solid black; font-weight: bold;">\</td>\
